...
//
// Vorsicht : Endekennung beim Empfangen, muss der Sender
...
//--------------------------------------------------------------
void uartGetInt(UART_NAME_t uart, uint16_t wert)
{
  uint16_t wr_pos;

  // byte an schreib position speichern
  wr_pos=UART_RX[uart].wr_ptr;
  // nur Ascii-Zeichen (und Endekennung) speichern
