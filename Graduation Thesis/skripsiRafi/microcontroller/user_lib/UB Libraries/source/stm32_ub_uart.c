...
void USART1_IRQHandler(void) {
	...
	if (USART_GetITStatus(USART1, USART_IT_RXNE) == SET) {
		...
		wr_pos=UART_RX[0].wr_ptr;

		UART_RX[0].rx_buffer[wr_pos]=USART_ReceiveData(USART1);


		if ((UART_RX[0].rx_buffer[wr_pos] == 'K') && (UART_RX[0].rx_buffer[wr_pos-1] == 'O'))
		{
			setMessage(0);
			setOk(true);
		}
		else if (UART_RX[0].rx_buffer[wr_pos] == '>')
		{
			setMessage(0);
			setOk(true);
		}
		...
		UART_RX[0].wr_ptr=wr_pos;

		// check ob Ueberlauf
		if(UART_RX[0].wr_ptr==UART_RX[0].rd_ptr) {
			UART_RX[0].status=RX_FULL;
		}
